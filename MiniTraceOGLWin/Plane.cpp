...
RayHitResult Plane::IntersectByRay(Ray& ray)
{
	...
	auto planeRayDotProduct = m_normal.DotProduct(ray.GetRay());
	//Continue only if the ray is not parallel with the plane
	if (planeRayDotProduct != 0)
	{
		auto a = ray.GetRayStart().DotProduct(m_normal) + m_offset;

		t = -(a / planeRayDotProduct);
